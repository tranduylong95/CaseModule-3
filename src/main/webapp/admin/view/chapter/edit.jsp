...
<html>
    ...
    <div class="wrapper">
        <jsp:include page="/admin/view/layout/header.jsp"></jsp:include>
                        ...
                        <section class="col-lg-12 connectedSortable">
                            ...
                            <div class="card card-info">
                                <div class="card-header">
                                    <h3 class="card-title">Chỉnh sửa Chapter</h3>
                                </div>
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form class="form-horizontal mb-0" action="/admin/comic${pathInfo}"  method="post" >
                                    <div class="card-body">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="inputEmail3" placeholder="Tên chapter" name="name" value="${chapter.getName()}">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-6">
                                                <select class="custom-select" name="lock" onchange="showTimePichker(this)">
                                                    <option value="0" <c:if test="${chapter.getLock()==0}">
                                                        selected </c:if>>Miễn phí</option>
                                                    <option value="1" <c:if test="${chapter.getLock()==1}">selected</c:if>>Khóa</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group row" id="time-picker" <c:if test="${chapter.getLock()==0}">
                                        ...
                                        <c:if test="${chapter.getLock()==1}">
                                            ...
                                            <div class="col-12 row" style="margin-bottom: 16px;">
                                                ...
                                                <div class="col-sm-6">
                                                    <input type="text" class="form-control text-right" placeholder="Giá tiền....." name="price"  <c:if test="${chapter.getLock()==1}">value="${chapter.getPrice()}"</c:if>">
                                                </div>
                                            </div>
                                            <div class="col-12 row" style="margin-bottom: 16px;">
                                                <div style="display: none" id="tableTextId" ><c:if test="${chapter.getLock()==1}"> ${DateUtils.ProcessTimeAfter(chapter.getTimeLock())} </c:if></div>
                                                <label for="inputEmail3" class="col-sm-2 col-form-label ">Time Lock</label>
                                                <div class="col-sm-10"  <c:if test="${chapter.getLock()==0}">data-toggle="modal" data-target="#exampleModal"</c:if>>
                                                    <div class="row">
                                                        <div class="hour col-lg-4 text-center" id="timelock-hours" style="position: relative;white-space: nowrap;">
                                                            <h4 >HOURS</h4>
                                                            ...
                                                            <c:if test="${chapter.getLock()==1}">
                                                            <div class="timepicker-op">
                                                                <span class="btn btn-primary"onclick="timePickerMinus()()" >-</span>
                                                                <span class="btn btn-primary" onclick="timePickerPlus()">+</span>
                                                            </div>
                                                            </c:if>
                                                            <input type="text" name="hoursChange" hidden>
                                                        </div>
                                                        <div class="minute col-lg-4 text-center" id="timelock-minutes">
                                                            <h4 >MINUTES</h4>
                                                            <div class="timepicker">
                                                                <span class="">0</span>
                                                            </div>
                                                        ...
                                                        <div class="second col-lg-4 text-center" id="timelock-seconds">
                                                            <h4>SECONDS</h4>
                                                            <div class="timepicker">
                                                                <span class="">0</span>
                                                            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="number" class="form-control text-right" max="999"  <c:if test="${chapter.getLock()==1}">value="${DateUtils.ProcessTimeAfterHour(chapter.getTimeLock())}"</c:if> min="1" name="hours" <c:if test="${chapter.getTimeLock()==0}">value="0"</c:if>>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="inputEmail3" class="col-sm-2 col-form-label ">Minute</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control text-right" max="59" min="0" name="minutes" <c:if test="${chapter.getLock()==1}">value="${DateUtils.ProcessTimeAfterMinus(chapter.getTimeLock())}"</c:if> <c:if test="${chapter.getTimeLock()==0}">value="0"</c:if>>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="inputEmail3" class="col-sm-2 col-form-label " >Second</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control text-right" max="59"  min="0" name="seconds" <c:if test="${chapter.getLock()==1}">value="${DateUtils.ProcessTimeAfterSecond(chapter.getTimeLock())}"</c:if>  <c:if test="${chapter.getTimeLock()==0}">value="0"</c:if>>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    document.getElementsByTagName("body")[0].onload=function(){saveInputTime()};
    ...
    function getTimeLockText(text){
        var x = setInterval(function(){
            var timeString = text.innerText;
    ...
    function changeText(element){
        ...
        var x=document.getElementsByClassName("timepicker");

        for (var i=0;i<x.length;i++){
            x[i].getElementsByTagName("span")[0].innerText=newText.charAt(i);
        }
        document.querySelector('input[name="hoursChange"]').value=parseInt(timeParts[0]);
        document.querySelector('input[name="minutesChange"]').value=parseInt(timeParts[1]);
        document.querySelector('input[name="secondsChange"]').value=parseInt(timeParts[2]);
    }
    function timePickerPlus() {
        var elementHidden = document.getElementById("tableTextId");
        var timeString = elementHidden.innerText;
        var timeParts = timeString.split(":");
        var hours = parseInt(timeParts[0]) + 1;
        if (hours > 999) {
            hours = 999;
        } else if (hours < 10) {
            hours = "00" + hours;
        ...
        elementHidden.innerText = hours + ":" +
            timeParts[1] + ":" +
            timeParts[2];
    }
    function timePickerMinus(){
        var elementHidden = document.getElementById("tableTextId");
        var timeString = elementHidden.innerText;
        var timeParts = timeString.split(":");
        var hours = parseInt(timeParts[0]) - 1;
